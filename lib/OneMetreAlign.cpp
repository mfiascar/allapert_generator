...
Aperture OneMetreAlign::GetAperture(double p)
{
  ...
  if ( !done ){
    ...
    if ( p == 0.0 ){
      ...
      k = 0;
      while ( Pos_ex[k] <= p && k < (int)Pos_ex.size() ) 
	k++;
      // Different apertures at the same location: jump to the next interval
      if (Pos_ex[k] == Pos_ex[k-1] && k < (int)Pos_ex.size()-2 )
	k++;
  ...
  for (int i = 0; i < (int)Pos_ex.size()-1; i++){
...
void OneMetreAlign::status(){
  ...
  if ( Pos_ex.size() > 0 ){
    ...
    cout.precision(4);
    for (int i = 0; i < (int)Pos_ex.size(); i++)
