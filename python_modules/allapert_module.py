...
def plot_apertures(old, new, ip, coordinate):
    ...
    ax1.set_title('IP' + ip_name + ' (Old allapert in blue)')
...
def plot_apertures_overlap(old, new, ip, coordinate):
    ...
    plt.plot(s_tot, var, 'b--', linewidth = 2)
    plt.plot(s_tot_new, var_new, 'r-')
    plt.xlim(position - 150, position + 150)
    plt.ylim(0, 0.40)
    plt.xlabel('s [m]')
    plt.ylabel(coord + ' [m]')
    plt.title('IP' + ip_name + ' (Old allapert in blue)')


    pp = PdfPages('overlap_allapert_ip'+ ip_name + '_' + coord +'.pdf')
    pp.savefig()
    pp.close()
